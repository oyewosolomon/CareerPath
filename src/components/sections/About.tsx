...
const AboutSection = () => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true);
          // Optional: Unobserve after first appearance if you don't need to track it further
  ...
  return (
    <div id="about" className="bg-white py-24" ref={sectionRef}>
      <div className="container mx-auto px-6">
        ...
        {/* Stats - Only animate if section is visible */}
        {isVisible && (
          <div className={`grid md:grid-cols-4 gap-8 mb-20 transform transition-all duration-1000 delay-300 ${
            ...
            {stats.map((stat, index) => (
              <div key={index} className="bg-gray-50 rounded-xl p-6 text-center hover:shadow-lg transition-all">
                <div className="bg-orange-100 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 text-[#ff8e5a]">
                  {stat.icon}
                </div>
        ...
        {/* Team - Only animate if section is visible */}
        {isVisible && (
